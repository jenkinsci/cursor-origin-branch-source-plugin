 ...
 * <p>Scenarios covered:
 * <ul>
 *   <li>3.1 REST-only indexing: no git operations → token is unrestricted
 *   <li>3.2.i MBP {@code checkout scm}: token must be scoped to the specific repo
 *   <li>3.2.ii Standalone project with {@code CpsScmFlowDefinition}: same scoping required
 *   <li>3.3 {@code @Library} controller clone: token need not be scoped
 *   <li>3.4 {@code withCredentials}/{@code withGit} open mode: credential must be open=true
 *   <li>3.4 {@code withCredentials} closed mode: must fail immediately when open=false
 * </ul>
 */
class TokenScopingTest extends MockOriginServerTestBase {

    private static final String OPEN_CREDS_ID = "origin-open-creds";
    ...
    // ── 3.1: REST-only indexing ──────────────────────────────────────────────

    /**
     * Scenario 3.1: branch indexing uses only REST API calls; the git server is never contacted.
    ...
    // ── 3.2.i: MBP checkout scm ─────────────────────────────────────────────

    /**
     * Scenario 3.2.i: when a multibranch pipeline Jenkinsfile runs {@code checkout scm} on an
    ...
    void multiBranchCheckoutScopedToRepo() throws Exception {
        ...
        String sha = mockGitServer.addRepo(OWNER, "checkout-repo", "main", Map.of("data.txt", "hello"));
    ...
    // ── 3.2.ii: standalone CpsScmFlowDefinition ─────────────────────────────

    /**
     * Scenario 3.2.ii: a standalone pipeline job using {@link CpsScmFlowDefinition} also needs a
    ...
    void standaloneProjectCheckoutScopedToRepo() throws Exception {
        ...
        String sha = mockGitServer.addRepo(
                OWNER, "standalone-repo", "main", Map.of("Jenkinsfile", "node('remote') { echo 'done' }"));
    ...
    // ── 3.3: @Library controller clone ──────────────────────────────────────

    /**
     * Scenario 3.3: when a pipeline requests a {@code @Library} whose source is an
    ...
    void libraryCloneOnControllerUsesUnrestrictedToken() throws Exception {
        ...
        String sha = mockGitServer.addRepo(OWNER, "lib-repo", "main", Map.of("vars/myStep.groovy", "def call() {}"));
    ...
    // ── 3.4: withCredentials / withGit open mode ────────────────────────────

    /**
     * Scenario 3.4.a: {@code withGit} (via {@code gitUsernamePassword} binding) running a git
    ...
    void withGitOpenModeSucceeds() throws Exception {
        ...
        addOpenCredentials(OPEN_CREDS_ID);

        WorkflowJob job = r.createProject(WorkflowJob.class, "with-git-test");
        String repoUrl = mockGitServer.baseUrl() + "/" + OWNER + "/git-repo.git";
    ...
    /**
     * Scenario 3.4.b: {@code withCredentials} with a {@code usernameColonPassword} binding running
     * a git clone succeeds when the credential has {@code open=true}.
     */
    @Test
    void withCredentialsOpenModeCloneSucceeds() throws Exception {
        ...
        // URL with credentials embedded: http://x-access-token:<token>@host/path.git
        // usernameColonPassword gives "username:password" in one variable
        String repoHost = mockGitServer.baseUrl().replaceFirst("http://", "");
        String repoPath = "/" + OWNER + "/clone-repo.git";
        job.setDefinition(new CpsFlowDefinition(
                "node('remote') {\n"
                        + "  withCredentials([usernameColonPassword(credentialsId: '"
                        + OPEN_CREDS_ID
                        + "', variable: 'CREDS')]) {\n"
                        + "    sh 'git clone http://$CREDS@"
                        + repoHost
                        + repoPath
                        + " cloned'\n"
                        + "  }\n"
    ...
    /**
     * Scenario 3.4.c: {@code withCredentials} with a {@code usernameColonPassword} binding running
     * {@code curl} against the git server succeeds when the credential has {@code open=true}.
     */
    @Test
    void withCredentialsOpenModeCurlSucceeds() throws Exception {
        mockGitServer.addRepo(OWNER, "curl-repo", "main", Map.of("file.txt", "hello"));
        addOpenCredentials(OPEN_CREDS_ID);

        WorkflowJob job = r.createProject(WorkflowJob.class, "curl-test");
        String infoRefsUrl = mockGitServer.baseUrl() + "/" + OWNER + "/curl-repo.git/info/refs?service=git-upload-pack";
        job.setDefinition(new CpsFlowDefinition(
                "node('remote') {\n"
                        + "  withCredentials([usernameColonPassword(credentialsId: '"
                        + OPEN_CREDS_ID
    ...
    // ── 3.4: withCredentials closed mode ────────────────────────────────────

    /**
     * Scenario 3.4 closed: using an {@code OriginAppCredentials} without {@code open=true} in a
